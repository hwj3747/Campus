...
public class EaseChatFragment extends EaseBaseFragment implements EMEventListener {
    ...
    protected static final int REQUEST_CODE_MAP = Toast.LENGTH_SHORT;
    ...
    static final int ITEM_TAKE_PICTURE = Toast.LENGTH_SHORT;
    ...
    public EaseMessageAdapter getMessageAdapter() {
        return messageList.getMessageAdapter();
    }
    ...
    protected void setUpView() {
        ...
        titleBar.setBackgroundColor(getResources().getColor(R.color.color_primary_green));
...
//        });
    ...
    protected void registerExtendMenuItem(){
        for(int i = Toast.LENGTH_SHORT; i < itemStrings.length; i++){
    ...
    protected void onConversationInit(){
        ...
        int msgCount = msgs != null ? msgs.size() : Toast.LENGTH_SHORT;
        if (msgCount < conversation.getAllMsgCount() && msgCount < pagesize) {
            String msgId = null;
            if (msgs != null && msgs.size() > Toast.LENGTH_SHORT) {
                msgId = msgs.get(Toast.LENGTH_SHORT).getMsgId();
            }
            if (chatType == EaseConstant.CHATTYPE_SINGLE) {
        ...
        swipeRefreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh() {
                new Handler().postDelayed(new Runnable() {
                    ...
                    @Override
                    public void run() {
                        if (listView.getFirstVisiblePosition() == Toast.LENGTH_SHORT && !isloading && haveMoreData) {
                            List<EMMessage> messages;
                            try {
                                if (chatType == EaseConstant.CHATTYPE_SINGLE) {
                                    messages = conversation.loadMoreMsgFromDB(messageList.getItem(Toast.LENGTH_SHORT).getMsgId(),
                                            pagesize);
                                } else {
                                    messages = conversation.loadMoreGroupMsgFromDB(messageList.getItem(Toast.LENGTH_SHORT).getMsgId(),
                                            pagesize);
                                }
                            } catch (Exception e1) {
                            ...
                            if (messages.size() > Toast.LENGTH_SHORT) {
                                messageList.refreshSeekTo(messages.size() - Toast.LENGTH_SHORT);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) { 
            if (requestCode == REQUEST_CODE_CAMERA) { // 发送照片
                ...
                double latitude = data.getDoubleExtra("latitude", Toast.LENGTH_SHORT);
                double longitude = data.getDoubleExtra("longitude", Toast.LENGTH_SHORT);
    ...
    protected void sendPicByUri(Uri selectedImage) {
        ...
        if (cursor != null) {
            cursor.moveToFirst();
            int columnIndex = cursor.getColumnIndex(filePathColumn[Toast.LENGTH_SHORT]);
            ...
            if (picturePath == null || picturePath.equals("null")) {
                ...
                toast.setGravity(Gravity.CENTER, Toast.LENGTH_SHORT, Toast.LENGTH_SHORT);
                toast.show();
                return;
            }
