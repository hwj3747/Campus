package com.haiyangroup.scampus.ui.approval;



import com.haiyangroup.scampus.common.AppComponent;
import com.haiyangroup.scampus.common.MyScope;
import com.haiyangroup.scampus.model.ApprovalModel;
import com.haiyangroup.scampus.present.ApprovalPresenter;

import compartment.HasPresenter;
import dagger.Component;

/**
 * 请假审批页面的dagger2注解配置
 * @author 何伟杰
 * @version 1.0, 2016/5/11
 */
@Component(
        dependencies = AppComponent.class, modules = ApprovalModel.class
)
@MyScope
public interface ApprovalComponent extends HasPresenter<ApprovalPresenter> {
    void inject(ApprovalFragment fragment);
}
