 ...
 */
@MyScope
public class AgendaAddPresenter extends BasePresenter<AgendaAddView> {
    @Inject
    AgendaAddModel model;

    @Inject
    AgendaAddPresenter() {
    }

    private Subscription mAgendaAddSubscription;

    @Override
    public void onDestroy() {
        super.onDestroy();
        if (mAgendaAddSubscription != null && !mAgendaAddSubscription.isUnsubscribed()) {
            mAgendaAddSubscription.unsubscribe();
        }

    }


    private Observer<AbsReturn<DefaultData>> AgendaAddObserver = new EndObserver<AbsReturn<DefaultData>>() {
        @Override
        public void onEnd() {
            if (getBaseView() != null) {
