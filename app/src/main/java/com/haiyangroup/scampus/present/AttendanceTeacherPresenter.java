 ...
 */
@MyScope
public class AttendanceTeacherPresenter extends BasePresenter<AttendanceTeacherView> {
    @Inject
    AttendanceTeacherModel model;

    @Inject
    AttendanceTeacherPresenter() {
    }

    private Subscription mAttendanceTeacherSubscription;

    @Override
    public void onDestroy() {
        super.onDestroy();
        if (mAttendanceTeacherSubscription != null && !mAttendanceTeacherSubscription.isUnsubscribed()) {
            mAttendanceTeacherSubscription.unsubscribe();
        }

    }


    private Observer<AbsReturn<DefaultData>> AttendanceTeacherObserver = new EndObserver<AbsReturn<DefaultData>>() {
        @Override
        public void onEnd() {
            if (getBaseView() != null) {
